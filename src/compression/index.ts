  ...
  return messages.reduce((total, msg) => {
    ...
    if (Array.isArray(msg.content)) {
      for (const part of msg.content) {
        ...
        else if (part.type === "image_url") chars += 2500; // ~1000 tokens worth
      }
    } else {
 ...
 */
function hasVisionContent(messages: NormalizedMessage[]): boolean {
...
export function shouldCompress(messages: NormalizedMessage[]): boolean {
  ...
  if (hasVisionContent(messages)) return false;
