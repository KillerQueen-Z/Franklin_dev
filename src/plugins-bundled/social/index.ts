...
import type { SocialConfig, ScoredPost, DraftReply } from './types.js';
  ...
  async buildConfigFromAnswers(
    ...
    if (platform === 'Reddit' || platform === 'Both') {
      try {
        ...
        if (Array.isArray(parsed)) subreddits = parsed;
      } catch { /* use empty */ }
    }
...
async function searchStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const sc = ctx.config as SocialConfig;
...
async function filterStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const results = (ctx.data.searchResults ?? []) as SearchResult[];
  const sc = ctx.config as SocialConfig;
  const product = sc.products?.[0];
  if (!product) return { summary: 'No product configured', abort: true };

  const relevant: Array<SearchResult & { relevanceScore: number }> = [];

  for (const post of results) {
    if (await ctx.isDuplicate(post.url)) continue;
...
async function scoreStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const posts = (ctx.data.filteredPosts ?? []) as Array<SearchResult & { relevanceScore: number }>;
  const sc = ctx.config as SocialConfig;
  const product = sc.products[0];
...
async function draftStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const posts = (ctx.data.scoredPosts ?? []) as ScoredPost[];
  const sc = ctx.config as SocialConfig;
  const product = sc.products[0];
  const drafts: DraftReply[] = [];

  for (const post of posts) {
    const tier = post.leadScore >= 7 ? 'premium' as const : 'cheap' as const;
    const maxLength = post.platform === 'reddit' ? sc.replyStyle.maxLengthReddit : sc.replyStyle.maxLengthX;

    const prompt = buildReplyPrompt(
      { title: post.title, body: post.snippet, platform: post.platform },
      { name: product.name, description: product.description },
      { tone: sc.replyStyle.tone, maxLength, rules: sc.replyStyle.rules },
    );
    ...
    try {
      ...
      ctx.log(`Failed to draft reply for ${post.url}: ${(err as Error).message}`);
    }
  }
...
async function previewStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const drafts = (ctx.data.drafts ?? []) as DraftReply[];
  if (drafts.length === 0) return { summary: 'No drafts to preview' };
...
async function postStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const drafts = (ctx.data.drafts ?? []) as DraftReply[];
  let posted = 0;
  ...
  for (const draft of drafts) {
    ...
    if (ctx.sendMessage) {
      try {
        ...
        ctx.log(`Failed to post to ${draft.post.platform}: ${(err as Error).message}`);
      }
    } else {
...
async function trackStep(ctx: WorkflowStepContext): Promise<WorkflowStepResult> {
  const drafts = (ctx.data.drafts ?? []) as DraftReply[];
  return {
